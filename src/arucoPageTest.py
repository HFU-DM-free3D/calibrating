...
#arucoDict = cv2.aruco.Dictionary(ARUCO_DICT[args["type"]], 50)
...
border_rest_x = A4_width - (x * 170 + (x - 1) * margin)
border_rest_y = A4_height - (y * 170 + y * text_size + (y - 1) * margin)
...
border_half_rest_x = int(np.floor(border_rest_x/2))
border_half_rest_y = int(np.floor(border_rest_y/2))
...
border_size_m = int(np.floor(170 * multiplier))
...
border_half_rest_x_m = int(np.floor(border_half_rest_x * multiplier))
border_half_rest_y_m = int(np.floor(border_half_rest_y * multiplier))

tag_id = args["id"]

border_thickness_pixels = 1
...
for i in range(0, y):
	for j in range(0, x):
		...
		borderVali = border_half_rest_y_m + i*border_size_m
		borderValj = border_half_rest_x_m + j*border_size_m
		print(i_val)
		print(borderVali)
		tag = np.zeros((size_m, size_m, 1), dtype="uint8")
		page[i_val - border_thickness_pixels:i_val + size_m + border_thickness_pixels,j_val - border_thickness_pixels:j_val + size_m + border_thickness_pixels] = 0
		...
		page[borderVali:borderVali+border_size_m,
            borderValj:borderValj + border_thickness_pixels] = 0  # Top border
		page[borderVali:borderVali + border_thickness_pixels,
            borderValj:borderValj + border_size_m] = 0  # Left border
		page[borderVali + border_size_m:borderVali + border_size_m + border_thickness_pixels,
            borderValj:borderValj + border_size_m] = 0 # right border
		page[borderVali:borderVali+border_size_m,
            borderValj+border_size_m:borderValj+border_size_m + border_thickness_pixels] = 0  # bottom border
		tag_id += 1
